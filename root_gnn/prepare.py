...
class WTaggerDataset(DataSet):
    ...
    def process(self, save=False, outname=None, n_evts_per_record=10, debug=False):
        ...
        def make_graph(event, debug=False):
            ...
            n_nodes = len(event) // self.n_node_features
            nodes = [[
                event[inode*self.n_node_features+0], # px
                event[inode*self.n_node_features+1], # py
                event[inode*self.n_node_features+2], # pz
                event[inode*self.n_node_features+3]  # E
            ] for inode in range(n_nodes) ]
            node_target = [
                event[inode*self.n_node_features+5] # isFromW
                for inode in range(n_nodes)
            ]
        ...
        with open(self.filename, 'r') as f:
            for line in f:
                ...
                self.graphs += make_graph(event, debug)
                self.n_evts += 1
                ievt += 1
                if save and ievt % n_evts_per_record == 0:
                    self.tot_data = len(self.graphs)
