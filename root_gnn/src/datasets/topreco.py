...
def make_graph(event, debug=False):
    ...
    nodes = np.array([[0.1*(inode+1)]* n_node_features for inode in range(n_max_tops)], dtype=np.float32)
    ...
    nodes = np.concatenate([nodes, jet_nodes], axis=0).astype(np.float32)
