...
def make_graph(chain, debug=False):
    ...
    scale_factors = np.array([1.0,1.0,1.0,1.0/3.0,1.0/math.pi,1.0,1.0,1.0,1.0],dtype=np.float32)
        ...
        for itower in range(chain.JetTowerN[ijet]):
            if(not cutoff or chain.JetTowerEt[tower_idx] >= 1.0):
                tower_nodes.append([0.0,math.log10(chain.JetPt[ijet]),\
                              ...
                              0.0,\
                              0.0,
                              math.log10(1.0+chain.TowerEem[tower_idx]),
                              math.log10(1.0+chain.TowerEhad[tower_idx])])
            tower_idx += 1
...
class TauIdentificationDatasetTrackvarsTowervars(DataSet):
    def __init__(self,
                 use_cutoff=False,\
                 track_limit=None,\
                 tower_limit=None):
