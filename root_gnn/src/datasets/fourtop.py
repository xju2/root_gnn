...
is_signal = False
max_btag_score = 5
eye_btag = np.eye(max_btag_score)
dummy_btag_scores = [0.] * max_btag_score
def make_graph(event, debug=False):
    ...
    # add one-hot encoding for particle type: jet, electron, muon
    # jet: [1, 0, 0], electron: [0, 1, 0]; muon: [0, 0, 1]
    ...
    nodes = [[
        ...
        1, 0, 0
    ] + eye_btag[event.PCBtag[idx]-1].tolist() for idx in range(n_jets)]
    ...
    if n_ele > 0:
        nodes += [[
            ...
            0, 1, 0
        ] + dummy_btag_scores for idx in range(n_ele)]
    ...
    if n_muon > 0:
        nodes += [[
            ...
            0, 0, 1
        ] + dummy_btag_scores for idx in range(n_muon)]
