...
def display_graph(event):
    ...
    # import the dataset desired
    from root_gnn.src.datasets.ditaumass import DiTauMassDataset
    dataset = DiTauMassDataset()
    
    graph = dataset.make_graph(event)
    if graph[0][0] is None:
        return None
    
    input_graph, output_graph = graph[0]
